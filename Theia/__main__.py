...
from Theia.utils import create_logger, show
from Theia.theia import Theia
...
def job(root_dir: str, sub_dir: str, interpreters: str, requirements: str, ip: str, pause: bool, token: str, clear: bool, url: str) -> None:
    ...
    logger_instance = create_logger('Theia', base_dir=theia_dir)
    show(logger_instance, f'Going from:\n\t\t{original_dir}\n\tto:\n\t\t{theia_dir}')
    os.chdir(theia_dir)

    t = Theia(theia_dir, interpreters)
    t.set_gitlab_runner(ip, token, url)
    t.update(path=requirements, clear=clear, pause=pause)


if __name__ == '__main__':
    main_parser = ArgumentParser("Theia Command Line Interface")
    main_parser.add_argument("--directory", "--root-dir", default=CI_FOLDER, dest="root_dir",
                             help="Base directory to use for Theia VENV.")
    main_parser.add_argument("--sub-dir", default="theia_venv", dest="sub_dir",
                             help="Sub directory to use for Theia VENV.")
